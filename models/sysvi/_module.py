...
class SysVAE(BaseModuleClass, EmbeddingModuleMixin):
    ...
    def _get_cov(
        ...
        * ``'cont'``: All covariates that are already continous.
        ...
        cov = {
            ...
            "continuous": torch.concat(cont_parts, dim=1)
            if len(cont_parts) > 0
            else None,
        }
        return cov
    ...
    def inference(
        ...
        result = self.encoder(
            x, batch_index=batch_index, cat_list=cat_covs, cont=cont_covs
        )
    ...
    def loss(
        ...
        # ).sum(dim=1)
        log_x_hat = generative_outputs["px"].loc
        log_theta = generative_outputs["px"].scale
        ...
        NLL_x = -pxz.log_prob(x_true)
        reconst_loss_x = NLL_x
    ...
    def random_select_batch(self, batch: torch.Tensor) -> torch.Tensor:
        ...
        new_batch_indices = available_batches[
            ...
            torch.randint(
                0, available_batches.shape[1], (batch.shape[0],), device=batch.device
            ),
        ]
    ...
    def sample(self, *args, **kwargs):
        ...
        raise NotImplementedError(
            "The use of decoded expression is not recommended for SysVI."
        )
